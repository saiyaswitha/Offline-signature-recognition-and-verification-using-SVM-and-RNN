...
genuine_image_filenames = listdir("data/valid")
forged_image_filenames = listdir("data/invalid")

genuine_image_paths = "data/valid"
forged_image_paths = "data/invalid"

genuine_image_features = []
forged_image_features = [] 

'''for name in genuine_image_filenames:
    signature_id = int(re.split("[_.]", name)[-3])
    with open('data/file.txt', 'a') as handle:
            handle.write(signature_id)
            handle.write('\n')
    print(signature_id)
    genuine_image_features[signature_id].extend({"name": name})'''
...
def surf(im, path, display=False):
    raw_image = cv2.imread(path)
...
for person in range(1, 56):
    ...
    for i in range(1,25):
        image_path = os.path.join(gpath, 'original_'+per+'_'+str(i)+'.png')
        preprocessed_image = preprocess_image(image_path)
        des_list.append(surf(preprocessed_image, image_path))
    ...
    for i in range(1,25):
        image_path = os.path.join(fpath, 'forgeries_'+per+'_'+str(i)+'.png')
        preprocessed_image = preprocess_image(image_path)
        des_list.append(surf(preprocessed_image, image_path))
...
im_features = np.zeros((24 + 24, k), "float32")
for i in range(48):
    ...
    #clf.fit((train_genuine_features), np.array( [1 for x in range(len(train_genuine_features))]))
